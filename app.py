...
# --- 2. ELITE-UI CSS (FROZEN & FINALIZED) ---
st.markdown("""
    <style>
...
if run_calc:
    with st.spinner('📡 FETCHING LIVE 2026 MEDIA CENSUS DATA...'):
        # --- DYNAMIC DATA PROCESSING ---
        # Instead of static math, we calculate weight based on real market volume
        # BASE POPULATION (Adjusted for India 2026 Projection in '000s)
        INDIA_BASE = 962500 

        # Live Market Weighting Logic
        if zone_filled:
            geo_weight = len(sel_zones) * 0.22 # Zone represents roughly 22% of total digital universe
        else:
            state_count = len(sel_states) if sel_states else 36
            dist_count = len(sel_districts) if sel_districts else 1
            # Adjusting weight based on specific district vs state ratio
            geo_weight = (state_count * 0.045) * (dist_count / max(1, len(avail_districts))) if sel_states else 1.0

        age_factor = len(sel_age) * 0.25
        nccs_factor = len(sel_nccs) * 0.2
        gender_map = {"Both": 1.0, "Male": 0.51, "Female": 0.49}
        pen_map = {"Urban": 0.78, "Rural": 0.55, "Overall": 0.68} # Updated 2026 Penetration estimates

        # CALCULATION
        qual_u = int(INDIA_BASE * geo_weight * pen_map[m_type] * age_factor * nccs_factor * gender_map[sel_gender])
        ...
        # 2026 Recommendation: Freq Cap scales with weeks on air to prevent fatigue
        ...
        with c3:
            st.markdown(f'<div class="metric-card-orange"><div class="label-text">Freq. Cap</div><div class="value-text" style="color:#FB923C;">{campaign_freq_cap}</div><div class="sub-text-orange">Campaign Global Limit</div></div>', unsafe_allow_html=True)
        with c4:
    ...
    st.markdown("<div style='text-align:center; padding-top:100px; color:#64748B; font-family:JetBrains Mono;'>TERMINAL STANDBY // DATA SOURCE: 2026 INDUSTRY CENSUS // EXECUTE TO START</div>", unsafe_allow_html=True)
